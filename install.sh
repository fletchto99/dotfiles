...
then
	...
	brew install binwalk docker docker-machine exiftool fcrackzip ffmpeg gdb gist git-lfs gnupg hashcat hub md5sha1sum nmap nvm pidof pinentry-mac pipenv python3 radare2 rbenv socat sqlmap volatility wpscan
fi
...
then
	...
	brew cask install 1password bettertouchtool datagrip discord etcher hyperdock iterm2 keybase meld microsoft-office runescape softu2f steam vlc whatsapp adobe-acrobat-reader binary-ninja deluge docker filezilla idafree java8 mactex messenger moom skim speedcrunch sublime-text vmware-fusion wireshark bartender burp-suite disablemonitor encryptme google-chrome intellij-idea keka megasync metasploit paragon-extfs plex-media-player slack spotify teamviewer webstorm zoomus
fi
